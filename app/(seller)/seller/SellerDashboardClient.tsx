  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-8 px-6 py-12">
      <PageHeader
        ...
        actions={!isLoading && !isApproved ? <LimeButton href="/seller/kyc">Complete KYC</LimeButton> : undefined}
      />
                ...
                <Stagger className="space-y-2">
                  {projects.map((p) => (
                    <StaggerItem key={p.id}>
                      <div className="flex items-center justify-between rounded-md border border-iron-filings bg-surface-raised p-4">
                        <div>
                ...
                <Stagger className="space-y-2">
                  {sales.map((s) => (
                    <StaggerItem key={s.id}>
                      <div className="rounded-md border border-iron-filings bg-surface-raised p-4">
                        <p className="font-nb-international-pro text-[length:var(--text-body)] !text-bone-vellum">
                          {s.listingTitle}
                        </p>
                        <p className="font-jetbrains-mono text-[12px] text-bone-vellum/70">
