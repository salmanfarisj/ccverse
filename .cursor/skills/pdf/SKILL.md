...
- **poppler** — required for `pdf2image` / `convert_pdf_to_images.py` (`brew install poppler` on macOS)
- **qpdf** — optional CLI merge/split/rotate (`brew install qpdf`)
- **tesseract** — optional OCR for scanned PDFs (`brew install tesseract`)

Run bundled scripts from this directory, e.g. `python scripts/check_fillable_fields.py <file.pdf>`.
